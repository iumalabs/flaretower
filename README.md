...
| `Access: Identity Providers Read` | `GET /accounts/{id}/access/identity_providers`                                                                                                                                                                     | Resolve an identity provider id to its human-readable name, for policy `login_method` rules and the applications table's Identity column (Module 3, specs/014-access-dashboard research.md §2). Not yet independently confirmed against a live token — every application in production currently shows `identity_summary: "— none —"`, which per `summarizeIdentity()`'s own logic is also what a genuinely policy-less-of-that-kind account looks like, so this is inconclusive either way rather than confirmed broken                                                                                                                                                                                                                                                                                             | Module 3                |
