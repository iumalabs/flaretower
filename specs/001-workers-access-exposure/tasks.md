...
- [ ] T001 Validate that Wrangler, the Cloudflare Vite plugin, and Playwright all run via Deno's
      ...
      appear.
- [ ] T002 Create root `deno.json` with the `npm:` import map (hono, jose, wrangler, react,
...
- [ ] T003 [P] Scaffold `worker/` and `app/` directories per plan.md's Project Structure (empty
...
- [ ] T004 [P] Create `wrangler.jsonc` at repo root: `workers_dev: false`, `preview_urls: true`
...
- [ ] T005 Implement Access JWT validation in `worker/auth/access-jwt.ts` using `jose`'s
      ...
      later phase.
- [ ] T006 [P] Create D1 migration `worker/db/migrations/0001_baseline.sql` for the
...
- [ ] T007 [P] Create D1 migration `worker/db/migrations/0002_exposure_findings.sql` for this
      module's `exposure_findings` and `exposure_alerts` tables per data-model.md.
- [ ] T008 Implement the Hono router skeleton in `worker/index.ts`'s `fetch` handler: requests to
      `/api/*` go to Hono (gated by T005's middleware), everything else falls through to
      `env.ASSETS.fetch(request)` (research.md §6).
- [ ] T009 Add an empty `scheduled` handler stub in `worker/index.ts` that logs invocation via
      `controller.cron` (research.md §5) — real logic wired in Phase 6 (US4).
- [ ] T010 [P] Configure `deno test` for `tests/unit/` and Playwright (via `npm:`) for `tests/e2e/`,
...
- [ ] T011 [P] [US1] Unit test in `tests/unit/evaluate.test.ts`: a Worker with multiple hostnames
...
- [ ] T012 [P] [US1] Playwright e2e test in `tests/e2e/exposure-inventory.spec.ts`:
...
- [ ] T016 [US1] Implement `POST /api/exposure/evaluate` in the same routes.ts: runs
...
- [ ] T017 [P] [US1] Build `app/pages/ExposureInventory.tsx` and
      ...
      values.
- [ ] T018 [US1] Wire `routes.ts` into the Hono router in `worker/index.ts`. Depends on T008, T015,
      T016.
...
- [ ] T019 [P] [US2] Unit test in `tests/unit/evaluate.test.ts`: a hostname with zero covering
...
- [ ] T020 [P] [US2] Playwright e2e test in `tests/e2e/exposure-inventory.spec.ts`: the critical
...
- [ ] T022 [US2] Style `ExposureStatusBadge`'s critical state using the design system's critical
...
- [ ] T023 [P] [US3] Unit test in `tests/unit/evaluate.test.ts`: an Access application with an
...
- [ ] T024 [P] [US3] Playwright e2e test in `tests/e2e/exposure-inventory.spec.ts`: the warning
      badge is visually distinct from both critical and safe.

### Implementation for User Story 3

- [ ] T025 [US3] Extend `inventory.ts` (T013) to fetch Access application policies
...
- [ ] T026 [US3] Surface the policy-openness `reason` string end-to-end through
...
- [ ] T027 [P] [US4] Unit test in `tests/unit/alerts.test.ts`: first-ever evaluation of a hostname
...
- [ ] T030 [US4] Wire the real `scheduled` handler in `worker/index.ts` (replacing T009's stub):
      ...
      T029.
- [ ] T031 [US4] Implement `GET /api/exposure/alerts` in routes.ts per contracts/api.md. Depends on
      T029.
- [ ] T032 [US4] Implement `POST /api/exposure/alerts/{id}/acknowledge` in routes.ts. Not a
...
- [ ] T034 [P] Document the exact required API token scopes (research.md §3's table) in the
